...
class AIFilter:
    ...
    def classify_batch(
        ...
        try:
            ...
            print(f"[AI Filter] classify request failed: {type(e).__name__}: {e}")
            if len(titles) <= 1:
                return []

            split_index = len(titles) // 2
            print(
                f"[AI Filter] batch split fallback: {len(titles)} items -> "
                f"{split_index} + {len(titles) - split_index}"
            )
            left = self.classify_batch(titles[:split_index], tags, interests_content)
            right = self.classify_batch(titles[split_index:], tags, interests_content)
            return left + right
