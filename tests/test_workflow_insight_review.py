...
class InsightReviewExportTest(unittest.TestCase):
    def test_export_writes_stage5_artifacts(self):
        with TemporaryDirectory() as tmp:
            ...
            selection = SelectionResult(strategy='ai', selected_items=list(snapshot.items), total_candidates=1, total_selected=1)
            insight = InsightResult(
                ...
                item_analyses=[InsightItemAnalysis(news_item_id='1', title='A', what_happened='A', diagnostics={'status': 'ok'})],
                diagnostics={
                    ...
                    'item_analysis_payloads': [{'news_item_id': '1', 'title': 'A', 'diagnostics': {'status': 'ok'}}],
            ...
            summary = export_insight_outbox(
                outbox_dir=tmp,
                generated_at=__import__('datetime').datetime(2026, 4, 20, 10, 0, 0),
